...
const DashboardPage: NextPage = () => {
  ...
  if (isZeirishi) {
    return (
      <ProtectedRoute>
        <div className="isolate bg-gray-50 py-24 px-10 sm:py-32 lg:px-8">
          {sale && isUpdated ? (
            <FakeForm todaySale={sale} daylySales={sales} />
          ) : (
            <AccessDeninedPage />
          )}
        </div>
  ...
  if (isAdmin) {
    return (
      <ProtectedRoute>
        <div className="isolate bg-gray-50 py-24 px-5 pb-52 sm:pb-36 lg:px-8">
          {isUpdated && (
          ...
          {sale && isUpdated && !nomalMode ? (
            ...
            <UpdateForm todaySale={sale} daylySales={sales} />
          ) : (
            <CreateFormContent
              year={year}
              month={month}
